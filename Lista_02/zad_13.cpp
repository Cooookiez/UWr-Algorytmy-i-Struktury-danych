...
{ 
	...
	node(int x0, node* p, node* l=nullptr, node* r=nullptr)
		:key(x0), parent(p) ,left(l), right(r)
	{}
};

void insert_recursive(node* &t, node* &p, int x) // wstawianie (rekurenycjnie)
{
	if(!t)
		t=new node(x, nullptr);
	else
		insert_recursive( x < t->key ? t->left : t->right, t->parent, x); // tail recursion 
}  
